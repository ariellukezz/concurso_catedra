...
<div>
    <div class="flex justify-between border-b-2" style="border-bottom: solid 2px #009DD1; padding-bottom: 8px;">
        <div><span style="font-weight: bold; font-size: 1rem; color:#009DD1;"> VII. Idioma (extranjero o nativo)* </span></div>
    ...
    <div class="mt-5 mx-2 my-2">
        <div class="mb-5" v-for="titulo in titulos" :key="titulo">
                    ...
                    <div class="flex justify-between" style="margin: -15px -10px;">
                        <div class="">
                            <div>
                                <span class="font-bold" style="font-size:1rem;"> {{  titulo.idioma }}</span>
                            </div>
                            <div>  {{ titulo.nivel }} </div>
                        </div>
        ...
        >
            <a-row :gutter="16">
                ...
                <a-col :xs="24" :sm="24" :md="24" :lg="24">
                    ...
                    <a-form-item name="idioma" :rules="[{ required: true, message: 'Este campo es obligatorio' }]">
                        <a-input v-model:value="form.idioma" style="height: 32px;">
                            <template #suffix>
                            </template>
                        </a-input>
                    </a-form-item>
                ...
                <a-col :xs="24" :sm="24" :md="24" :lg="24">
                    ...
                    <a-form-item name="nivel" :rules="[{ required: true, message: 'Este campo es obligatorio' }]">
                        <a-select
                            ref="select"
...
const eliminar = async (id) => {
    try {
        const res = await axios.get(`/eliminar-idioma/${id}`);
        if (res.data.estado) {
            await getIdiomas();
        } else {
...
const getIdiomas = async () => {
    try {
        const response = await axios.get('/get-idiomas');
...
const cargarDatos = async () => {
    ...
    try {
        ...
        const response = await axios.post("save-idioma", formData, {
            headers: {
