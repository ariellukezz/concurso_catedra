...
<div>
    <div class="flex justify-between border-b-2" style="border-bottom: solid 2px #009DD1; padding-bottom: 8px;">
        <div><span style="font-weight: bold; font-size: 1rem; color:#009DD1;"> IX. Actividades de extensión cultural, proyección y
            responsabilidad social en la especialidad </span></div>
        <div style="margin-top: -5px;"><a-button @click="abrirModal()">Agregar</a-button></div>
    </div>

    <div class="mt-5 mx-2 my-2">

        <div class="mb-5" v-for="titulo in titulos" :key="titulo">

            <div class="flex justify-between" style="margin: -5px -10px;">
                <div class="">
                    <div> <span class="font-bold" style="font-size:.9rem;"> {{  titulo.actividad }} </span>
                    </div>
                    <div><span class="italic">( {{ titulo.resolucion }} )</span></div>
                </div>
...
<div class="px-4">
    <a-modal v-model:open="modaltitulo" width="580px" title="Registro de actividad">
        <a-form
            ref="formDatos"
            name="form"
            :model="form" 
        ...
        >
            <a-row :gutter="16">
                <a-col :xs="24" :sm="24" :md="24" :lg="24">
                    <label>Actividad<span style="color:red;">*</span></label>
                    <a-form-item name="actividad" :rules="[{ required: true, message: 'Este campo es obligatorio' }]">
                        <a-input v-model:value="form.actividad" style="height: 32px;">
                            <template #suffix>
                            </template>
                        </a-input>
                    </a-form-item>
    ...
    <a-modal v-model:open="modalPDF" width="800px" title="Ver adjunto">
        <a-row :gutter="16">
            <a-col :xs="24" :sm="24" :md="24" :lg="24">
...
const titulos = ref([]);
const tipos = ref([
    { value:1, label:"Resolución Rectoral"},
    { value:2, label:"Resolución Directoral"},
    { value:3, label:"Resolución Decanal"}
]);
...
const eliminar = async (id) => {
    try {
        const res = await axios.get(`/eliminar/${id}`);
        if (res.data.estado) {
...
const getActividades = async () => {
    try {
        const response = await axios.get('/get-actividades  ');
...
const save = async () => {
    ...
    try {
        ...
        const response = await axios.post("save-actividad-proyeccion", formData, {
            headers: {
