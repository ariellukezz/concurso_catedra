<template>
    ...
    <div>
        <div class="flex justify-between border-b-2" style="border-bottom: solid 2px #009DD1; padding-bottom: 8px;">
            <div><span style="font-weight: bold; font-size: 1rem; color:#009DD1;"> X. Bonificación </span></div>
        ...
        <div class="mt-5 mx-2 my-2">
            ...
            <div class="mb-5" v-for="titulo in titulos" :key="titulo">
                ...
                <div class="flex justify-between" style="margin: -5px -10px;">
                    <div class="">
                        <div> <span class="font-bold" style="font-size:.9rem;"> {{  titulo.descripcion }} </span>
                        </div>
                        <div><span class="italic">( {{ titulo.tipo }} )</span></div>
                    </div>
    ...
    <div class="px-4">
        <a-modal v-model:open="modaltitulo" width="580px" title="Registro de bonificación">
            <a-form
                ref="formDatos"
                name="form"
                :model="form" 
    ...
    const titulos = ref([]);
    const tipos = ref([
        { value:1, label:"Profesionales RENACYT "},
        { value:2, label:"Ganador de Concurso Público"}
    ]);
    ...
    const getBonificaciones = async () => {
        try {
            const response = await axios.get('/get-bonificaciones');
    ...
    const save = async () => {
        ...
        try {
            ...
            const response = await axios.post("save-bonificacion", formData, {
                headers: {
