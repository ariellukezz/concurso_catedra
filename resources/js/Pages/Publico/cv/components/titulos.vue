...
<div>
    <div class="flex justify-between border-b-2" style="border-bottom: solid 2px #009DD1; padding-bottom: 8px;">
        <div><span style="font-weight: bold; font-size: 1rem; color:#009DD1;">I. Títulos y grados </span></div>
        ...
        >
            <a-row :gutter="16">
                ...
                <a-col :xs="24" :sm="24" :md="24" :lg="6">
                    <label>Reg en SUNEDU<span style="color:red;">*</span></label>
                    <a-form-item name="reg_sunedu" :rules="[{ required: true, message: 'Este campo es obligatorio' }]">
                        <a-select
                            ref="select"
                            v-model:value="form.reg_sunedu"
                            style="width: 100%"
                            :options="opcionesSUNEDU"
                            @focus="focus"
                            @change="selecionarTipo"
                        >
                        </a-select>
...
const cargarDatos = async () => {
    ...
    formData.append('fec_expedicion',  format(new Date(form.fec_expedicion), 'yyyy-MM-dd'));
