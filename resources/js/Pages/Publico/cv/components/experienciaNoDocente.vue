...
<div>
    <div class="flex justify-between border-b-2" style="border-bottom: solid 2px #009DD1; padding-bottom: 8px;">
        <div><span style="font-weight: bold; font-size: 1rem; color:#009DD1;"> IV. Experiencia profesional no docente universitaria </span></div>
    ...
    <div class="mt-5 mx-2 my-2">
        <div class="mb-5" v-for="titulo in titulos" :key="titulo">
                <div class="flex justify-between" style="margin: -15px -10px;">
                    <div class="">
                        <div> <span class="font-bold" style="font-size:1rem;"> {{  titulo.cargo }} </span> </div>
                        <div class="ml-0">
                            <div><span >  {{ titulo.institucion }} </span></div>
                            <div>
                            ...
                            <div><span class="italic" style="font-size: .8rem;"> ( Duración {{leertiempo(titulo.duracion) }} )</span></div>
                        </div>
                    </div>
    ...
    <a-modal class="lg:w-50" v-model:open="modaltitulo" title="Registro de investacion" @ok="handleOk">
        ...
        >
            <a-row :gutter="16">
                ...
                <a-col :xs="24" :sm="24" :md="24" :lg="24">
                    <label>Tiempo en meses<span style="color:red;">*</span></label>
                    <a-form-item name="duracion" :rules="[{ required: true, message: 'Este campo es obligatorio' }]">
                        <a-input-number
                            v-model:value="form.duracion"
                            :min="0"
                            style="width: 100%;"
                            :max="60"
                        />
...
const eliminar = async (id) => {
    try {
        const res = await axios.get(`/eliminar-experiencia-no-docente/${id}`);
        if (res.data.estado) {
            await getTitulos();
        } else {
